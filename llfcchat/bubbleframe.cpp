...
#include <QPainter>
#include <QDebug>
const int WIDTH_SANJIAO  = 8;  //三角宽（气泡后面的小三角的宽度）
    ...
    ,m_margin(3)
{
    ...
    if(m_role == ChatRole::Self)
        m_pHLayout->setContentsMargins(m_margin, m_margin, WIDTH_SANJIAO + m_margin, m_margin);//左上右下的距离
    else
...
{
    Q_UNUSED(margin);//避免警告
}


//应该指的是小三角前面的那一部分气泡所在位置
...
void BubbleFrame::paintEvent(QPaintEvent *e)
{
    ...
    painter.setPen(Qt::NoPen);//不用画笔，画出的东西没有边框

    if(m_role == ChatRole::Other)
    {
        //画气泡
        QColor bk_color(Qt::white);//别人发的置为白色
