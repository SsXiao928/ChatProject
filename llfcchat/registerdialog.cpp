...
{
//注册获取验证码回包的逻辑
    //能保证Dialog在生命周期中不会被删掉，所以可以捕获this
    //this代表指向该对象的指针，而不是该对象本身
    //map中注册的值为一个需要以const QJsonObject为参数的函数对象
...
void RegisterDialog::showTip(QString str, bool bo_ok)
{
    if(bo_ok){
        ui->err_tip->setProperty("state", "normal");
    }else{
...
void RegisterDialog::ChangeTipPage()
{
    ...
    //每次定时器计时到设定的时间后，定时器会触发 timeout 信号
